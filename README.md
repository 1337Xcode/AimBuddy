...
The Android app layer is Kotlin. The performance-critical runtime is native C++ through JNI.
...
- Modes:
  - Visual Assist Mode: no root required
  - Assisted Input Mode: root required for input injection only

## Runtime Modes

- Visual Assist Mode (no root): capture, inference, tracking, and on-screen overlay guidance.
- Assisted Input Mode (root optional): visual assist features plus low-latency input injection.
...
4. Start processing from the app UI.

### 4) Train and Export Model (Optional)
